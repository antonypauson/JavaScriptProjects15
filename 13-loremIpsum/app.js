...
form.addEventListener("submit", (event) => {
    ...
    //amount is the form's input area
    const value = parseInt(amount.value); //this wil be a string, so convert
    ...
    //value can be negative
        ...
        output.innerHTML = `<p>Enter a valid number!</p>`;
    } 
    else {
        let myText = text.slice(0, value);
        myText = myText.map(each => {
            return `<p class='result'>${each}</p>`;
        })
        console.log(myText.join(''));
        
        output.innerHTML = myText.join('');

    }
    //value can be more than 9 (more than array size)
})